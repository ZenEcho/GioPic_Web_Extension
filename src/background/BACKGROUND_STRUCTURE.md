# Background Script Architecture (v2.3.2)

## 1. 简介 (Introduction)

Background Service Worker 是浏览器扩展的“后端”，负责处理长期运行的任务、跨页面通信、系统级 API 调用（如右键菜单、通知、Web 请求拦截）。在 v2.3.2 版本中，进一步增强了 Side Panel 的状态同步与兼容性处理。
